...
/* Work around strong type checking. */
union {
    struct symbol *psym;
    struct varent *pvar;
    struct funent *pfun;
...
/*	If at beginning of string, must ask for more input	*/
    if (pline == line) {
        ...
        if (takptr < 0) {    /* no take file active: prompt keyboard input */
            printf("* ");
        }
...
/* Test for numeric input */
    if ((ISDIGIT(*pline)) || (*pline == '.')) {
        ...
        qnc = 0.0;
        if (*pline == '0') { /* leading "0" may mean octal or hex radix */
            ++pline;
            if (*pline == '.')
                goto decimal; /* 0.ddd */
        ...
        int echo;
{
    ...
    if (takptr < 0) {    /* get character from keyboard */
/*
if DECPDP
		gtlin( gline );
		return(0);
...
*/
        *pline = getchar();
/*endif*/
    } else {    /* get a character from take file */
        i = fgetc(takstk[takptr]);
        if (i == -1) {    /* end of take file */
            if (takptr >= 0) {    /* close file and bump take stack */
